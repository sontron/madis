...
if(!all(is.na(myGlmParms$data))){
  ...
  cat('\n')
    ...
    if(myGlmParms$Family[i]=='binomial'){
      ...
      fitSum$coefficients<-cbind(fitSum$coefficients,OR=exp(fitSum$coefficients),exp(confint(res$glmResFull)))
    ...
    cat('\n')
     ...
     if(myGlmParms$Family[i]=='binomial'){
      ...
      fitSum$coefficients<-cbind(fitSum$coefficients,OR=exp(fitSum$coefficients),exp(confint(res$glmResStep)))
      # cat(pander(exp(confint(res$glmResFull))))
      cat(pander::pander(fitSum,split.table=Inf))
      cat('\n')
    } else {
