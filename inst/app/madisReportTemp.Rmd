...
if(is.na(descParams$xvars)) {
  ...
  for(i in 1:nrow(descParams)){
    ...
    cat(pander(res$resDesc,split.table=Inf))
    cat('\n')
...
if(is.na(hTestParams$xvars)){
  ...
  for(i in 1:nrow(hTestParams)){
    ...
    cat(pander(res$hTestRes,split.table=Inf))
    cat('\n')
    plot(res$hTestGraph)
    cat('\n')
  }
...
if(is.na(myGlmParms$data)){
  ...
  for(i in 1:nrow(myGlmParms)){
    ...
    if(myGlmParms$Family[i]=='binomial'){
      ...
      cat(pander(fitSum,split.table=Inf))
      cat('\n')
    } else {
      
    cat('\n')
    cat(pander(summary(res$glmResFull),split.table=Inf))
    cat('\n')
    ...
    cat('\n')
    cat(pander(summary(res$glmResStep),split.table=Inf))
    cat('\n')
...
if(is.na(myCoxParms$data)){
  ...
  for(i in 1:nrow(myCoxParms)){
    ...
    cat(paste0('#### 第',i,'个模型的全模型结果：'))
    cat('\n')
    cat(pander(summary(res$coxResFull),split.table=Inf))
    cat('\n')
    ...
    cat(paste0('#### 第',i,'个模型的逐步回归模型结果：'))
    cat('\n')
    cat(pander(summary(res$coxResStep),split.table=Inf))
    cat('\n')
...
if(is.na(myLmeParms$data)){
  ...
  for(i in 1:nrow(myLmeParms)){
    ...
    cat(paste0('#### 第',i,'个模型的全模型结果：'))
    cat('\n')
    cat(pander(res$lmeResFull,split.table=Inf))
    cat('\n')
    cat('\n')
    cat(paste0('#### 第',i,'个模型的逐步回归模型结果：'))
    cat('\n')
    cat(pander(res$lmeResStep,split.table=Inf))
    cat('\n')
    cat('\n')
    cat('\n')
  }
...
if(is.na(mypcaParms$data)){
  ...
  for(i in 1:nrow(mypcaParms)){
    ...
    cat('\n')
    cat(pander(res$resPCA$loadings[],split.table=Inf))
    cat('\n')
    cat('\n')
    cat(pander(res$cumVar,split.table=Inf))
    cat('\n')
  }
...
if(is.na(myfaParms$data)){
  ...
  for(i in 1:nrow(myfaParms)){
    ...
    cat('\n')
    cat(pander(res$resFA$loadings[],split.table=Inf))
    cat('\n')
    cat('\n')
    cat(pander(res$cumVar,split.table=Inf))
    cat('\n')
...
if(is.na(myProphetParms$data)){
  ...
  for(i in 1:nrow(myProphetParms)){
    ...
    cat('\n')
    cat(pander(res$tabRes,split.table=Inf))
    cat('\n')
...
if(is.na(myTableParms$data)){
  ...
  for(i in 1:nrow(myTableParms)){
    ...
    cat('\n')
    cat(pander(descTab(Formula=myTableParms$Formula[i],data=LstMadis$Data[[myTableParms$data[i]]]),split.table=Inf))
    cat('\n')
    cat('\n')
